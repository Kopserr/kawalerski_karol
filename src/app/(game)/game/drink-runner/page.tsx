...
import { DrinkRunnerGame } from "@/components/games/drink-runner/DrinkRunnerGame";
...
export default async function DrinkRunnerPage() {
  const data = await getBoardData();

  // Local-mode / no-backend fallback: let the game run — the store starts
  // in "local" mode regardless of what this page decides.
  const challengesDone = data ? countChallengesDone(Object.values(data.tileStates)) : 6;
  const minigame = data?.minigames.find((m) => m.key === "drink-runner") ?? getMinigameByKey("drink-runner")!;
    ...
    return (
      <main className="mx-auto flex h-dvh w-full max-w-[520px] flex-col items-center justify-center gap-4 bg-void px-6 text-center safe-x safe-top safe-bottom">
        <GlowCard className="flex w-full flex-col items-center gap-4 p-8">
          ...
          <h1 className="font-heading text-2xl">Drink Runner zablokowany</h1>
  ...
  return <DrinkRunnerGame groomPhoto={data?.groomPhoto ?? null} bridePhoto={data?.bridePhoto ?? null} />;
}
